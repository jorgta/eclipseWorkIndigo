...
public class ProcessAdminDeleteAction extends ActionSupport  {
			...
			try 
			  {
				...
				else
				{
					...
					if (process.equals("companyDelete"))
					  {
						
		                
						BeanCompany bean = (BeanCompany) session.get( BeanCompany.class, new Integer( getId().toString() ) );
						...
						{
							httpSession.setAttribute("done", new String("Companñia borrada correctamente" ));
						}
					  ...
					  {
						BeanUser bean = (BeanUser) session.get( BeanUser.class, new Integer( getId().toString() ) );		                
		                bean.setActive(0);
