...
function populateCalendar(year,month,day,hourStartTimeHour,hourEndTime,asunto)
{
	...
	while ( iter.hasNext() )
	  {
		...
		c.setTime( beanQuote.getDate_event() );

		out.write("populateCalendar(" 
				  + c.get( Calendar.YEAR) 
				  + ", " + ( c.get( Calendar.MONTH ) + 0 ) 
				  + ", " + c.get( Calendar.DATE) 
				  + ", " + beanQuote.getId_hour().getId()
				  + ", " + beanQuote.getHourQuantity() 
				  + ", '" + beanQuote.getList_description() + ": " + beanQuote.getId_client().getName() + "'"
				  + ");"
				 );
		out.write("\n");
	  }
