...
import com.eventAdmin.forms.RptPaymentsForm;

/**
 * @version 	1.0
 * @author		Bituos Tools S de RL MI
 */
public class RptPaymentsAction extends Action {
	...
	public ActionForward execute(
		...
		RptPaymentsForm rptPaymentsForm = (RptPaymentsForm) form;
			...
			try 
			  {
				...
				if(rptPaymentsForm.getProcess().equals("selectSale"))
				  {
					  query = " FROM BeanSale u" +
		        	          " WHERE u.id_company = " + beanUser.getId_company().getId() +
		        	          " AND u.id = " + rptPaymentsForm.getId_sale();
					  ...
					  if ( list.size() > 0 )
					    {
						  ...
						  httpSession.setAttribute( "crystal_rptname", "rptPay.rpt" ) ;								
							 
						  httpSession.setAttribute( "parameterCount", new Integer( 1 ) ) ;
												
						  httpSession.setAttribute( "parameter1", new Integer(  rptPaymentsForm.getId_sale() ) );
						 ...
						 /* BeanSale beanSale = (BeanSale) session.load( BeanSale.class, new Integer(rptPaymentsForm.getId_sale()));
						 
						  
						  
						  httpSession.setAttribute("id_sale", rptPaymentsForm.getId_sale() );						  
				...
				else if(rptPaymentsForm.getProcess().equals("payItMyAccount"))
				  {
					BeanSale beanSale = (BeanSale) session.load( BeanSale.class, new Integer(rptPaymentsForm.getId_sale()));
					double total = beanSale.getTotal();
					...
					beanPayments.setQuantity(Double.parseDouble( rptPaymentsForm.getQuantity() ));
					beanPayments.setDate_reg(Utils.StrToDate( rptPaymentsForm.getDate_reg()));
					beanPayments.setCanceled('N');
					//beanPayments.setDate_canceled(date_canceled);
					
					BeanTypePayment beanTypePayment = (BeanTypePayment) session.load( BeanTypePayment.class, new Integer(rptPaymentsForm.getId_type_payment()));	
					beanPayments.setId_tipo_pago(beanTypePayment);
					BeanPaymentTypeForm beanPaymentTypeForm = (BeanPaymentTypeForm) session.load( BeanPaymentTypeForm.class, new Integer(rptPaymentsForm.getId_payment_type_form()));
