			...
			try 
			  {
					...
					else if (process.equals("hotelsChange")) 
					 {
						httpSession.removeAttribute("done");
						httpSession.removeAttribute("error");
						...
						if (new SessionUtil().prepareDataForHotelChange(request,beanUser,session) == 1) 
						 {  				
						  ...
						  {				 
							  ...
							  forward = "hotelChangeError";
						  }
					 }
					else if (process.equals("hotelsDelete"))
					 {
						httpSession.removeAttribute("done");
						httpSession.removeAttribute("error");
						if (new SessionUtil().prepareDataForHotelDelete(request,beanUser,session) == 1) 
						  {								   			
							   forward = "hotelsDelete";
						  }
						 else
						  {
						       forward = "hotelDeleteError";
						  }
					 }
					else if (process.equals("hotelsActivate"))
					 {								
						httpSession.removeAttribute("done");
						httpSession.removeAttribute("error");
					     if (new SessionUtil().prepareDataForHotelActivate(request,beanUser,session) == 1) 
							 {
							   ...
							   forward = "hotelsActivate";
							}
						 else
							 {								  
		
							   forward = "hotelActivateError";
							 }
					 }
