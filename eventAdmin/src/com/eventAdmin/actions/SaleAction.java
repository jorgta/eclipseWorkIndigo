			...
			try 
			  {
				...
				else if( saleForm.getProcess().equals("selectedSale") )
				  {					
					boolean showError = false;
					BeanSale beanSale = (BeanSale) session.load( BeanSale.class, new Integer(saleForm.getId_sale()) );
					if ( !showError)
					  { 
						httpSession.setAttribute( "selectedSale", beanSale ) ;
					  }
