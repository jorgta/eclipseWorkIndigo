				...
				try
				  {
					...
					if(userChangeForm.getProcess().equals("updateUser"))
					  {
					...
					else if(userChangeForm.getProcess().equals("selectUser"))
					  {
						
						BeanUser bean = (BeanUser) session.load( BeanUser.class, new Integer( userChangeForm.getId() ) );
						
						httpSession.removeAttribute("currentBean");
						httpSession.setAttribute("currentBean",bean);
						...
						//userChangeForm.setCompany(bean.getId_company().getId()));
						...
						httpSession.setAttribute("done", new String("/eventAdmin/client/userSelect.jsp"));
						forward = mapping.findForward("ok");
					  }
				  }
				catch( Throwable  m)
