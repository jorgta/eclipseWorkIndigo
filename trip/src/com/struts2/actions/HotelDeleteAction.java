...
public class HotelDeleteAction extends ActionSupport  {
	...
	public void validate() {	
    	...
    	if(process.equals("hotelsDelete"))
    	{
			...
			try 
			  {
				...
				else
				{
					if (process.equals("hotelsDelete"))
					  {
						BeanHotel bean = (BeanHotel) session.load( BeanHotel.class, new Integer( getId().toString() ) );
		                
		                bean.setActive(0);
						tx = session.beginTransaction();
						session.update( bean );
						tx.commit();
						httpSession.setAttribute("done", new String("Hotel eliminado"));
						
						if (new SessionUtil().prepareDataForHotelDelete(request,beanUser,session) != 1) 
						{  				
					    	forward = "error";
						}
						else
						...
						if (process.equals("DeleteHotels"))
						 {
							if(request.getParameterValues("hotels")!= null)
							{
								...
								if(hotelSelected.length > 1)
									httpSession.setAttribute("done", new String("Hoteles borrados"));
								else
									httpSession.setAttribute("done", new String("Hotel borrado"));
								...
								httpSession.setAttribute("error", "Debe seleccionar los hoteles a eliminar");
