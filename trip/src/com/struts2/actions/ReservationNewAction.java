		...
		try 
		  {
			...
			else
			{
			     ...
			     if (process.equals("selectedRooms"))
				 {
					    	...
					    	if(roomsSelected.length > 1)
					    	{
								...
								if ( !list2.isEmpty() )
								{
								     ...
								     if(room.getId_hotel().getId()== first_beanRoom.getId_hotel().getId())
								     {
								    	...
								    	{
								    		httpSession.setAttribute("error","Seleccionó las habitaciones en fechas distintas");
								    	    forward = "error";
								    	    break;
								    	}
								     }
								     ...
								     {
								    	 httpSession.setAttribute("error","Seleccionó las habitaciones en distintos hoteles");
								    	 forward = "error";
								    	 break;
								     }
					...
					{  
						httpSession.setAttribute("error","No seleccionó las habitaciones");
					}
				 }
				else
				 if(process.equals("addReservation"))
									...
									for (int i = 0; i < listGuestAdd.size(); i++) 
									   {
										...
										else
										{
									    	...
									    	httpSession.removeAttribute("listGuestAdd");
											httpSession.setAttribute("listGuestAdd",listGuestAdd);
											//httpSession.setAttribute("done","El huésped se ha agregado con éxito");
													...
													if(listroomSelected.size()!= 0)
										    		 {
														 ...
														 httpSession.setAttribute("Prices_formule", "Precio total=(Cantidad de Adultos * Tarifa de adultos)+ (Cantidad de babies*Tarifa de babies)+(Cantidad de básicos*Tarifa de básicos)+(Cantidad de Child*Tarifa de child)+(Cantidad de Junior*Tarifa de Junior)");
