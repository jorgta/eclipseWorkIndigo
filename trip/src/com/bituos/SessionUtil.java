...
public class SessionUtil {
	public int prepareDataForPagination( HttpServletRequest req, BeanUser beanUser ,Session session, String listName, String BeanName, int elementsByPage, int currentPage )
	  { 
		...
		try
		  {
			...
			httpSession.removeAttribute("pagesList");
			...
			String query =  " FROM "+ BeanName +" u" +
							" WHERE u.id_company = " + beanUser.getId_company().getId() +
							" LIMIT " + String.valueOf(currentPage) + ", " + String.valueOf(elementsByPage);
			...
			if (!list.isEmpty()) 
			{  
				httpSession.setAttribute("pagesList",list);
				return 1;
			}
			else
			  {			 
	...
	public int prepareDataForHotelChange( HttpServletRequest req, BeanUser beanUser ,Session    session )
	  {
		HttpSession httpSession = req.getSession();
		try
		  {
			httpSession.removeAttribute("hotelsChangeBeanList");
			...
			if (!list.isEmpty()) 
			{  
				List<BeanHotel> hotelsChangeBeanList = new ArrayList<BeanHotel>();
				for (int i = 0; i < list.size(); i++) 
				{
					BeanHotel beanHotel = (BeanHotel)list.get(i);
				
					String query2 =  " FROM BeanRoom r"+ 
								     " WHERE r.id_hotel.id = " + beanHotel.getId()+
	                                 " AND r.active = 1" +
					...
					if (list2.isEmpty()) 
					{ 
						hotelsChangeBeanList.add(beanHotel);
					}
				}
				httpSession.setAttribute("hotelsChangeBeanList",hotelsChangeBeanList);
				return 1;

			}
			else
			  {
				httpSession.setAttribute("information", "No hay hoteles registrados para ser modificados");
				return 0;
			  }
	...
	public int prepareDataForHotelDelete( HttpServletRequest req, BeanUser beanUser ,Session    session )
	  {
		...
		try
		  {
			...
			httpSession.removeAttribute("listHotelsDelete");
			//httpSession.removeAttribute("done");
		    httpSession.removeAttribute("error");
		    httpSession.removeAttribute("information");
		    httpSession.removeAttribute("attention");
			...
			if (!list.isEmpty()) 
			{  
				List<BeanHotel> listHotelsDelete = new ArrayList<BeanHotel>();
				for (int i = 0; i < list.size(); i++) 
				{
					BeanHotel beanHotel = (BeanHotel)list.get(i);
					...
					if (list2.isEmpty()) 
					{ 
						listHotelsDelete.add(beanHotel);
					}
				}
				httpSession.setAttribute("listHotelsDelete",listHotelsDelete);
				return 1;
			}
			else
			  {			 
	...
	public int prepareDataForlistRoom( HttpServletRequest req, BeanUser beanUser ,Session    session , BeanHotel beanHotel)
	  {
		HttpSession httpSession = req.getSession();
		try
		  {
		    ...
		    String query =   " FROM BeanRoom r" +
		                     " WHERE r.id_hotel.id = " + beanHotel.getId()+
		                     " AND r.id_hotel.id_company.id="+ beanUser.getId_company().getId()+
		                     " AND r.active = 1";
			...
			{  
			  ...
			  {
				httpSession.setAttribute("information", "No hay habitaciones registradas para este hotel");
				return 0;
			  }
		...
		try
		  {
		    ...
		    httpSession.removeAttribute("roomsReservation2Change");
		    
		    httpSession.setAttribute("module","pre");
		    
		    String query2 = " FROM BeanReservation_Rooms rr"+
				            " WHERE rr.id_reservation.id_type_status_reservation <> 3" +
				            " AND rr.id_room.id_hotel.id_company.id=" + beanUser.getId_company().getId() ; 
