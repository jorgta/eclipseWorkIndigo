	...
	<logic:present name="beanUser" scope="session">
		  ...
		  <logic:present name="processList" scope="session">
			  <%
		       ...
		       BeanDoctor  beanDoctor =(BeanDoctor)ses.getAttribute( "beanDoctorSelectedForChange" );
		        
		       BeanRepresentative  beanRepresentative =(BeanRepresentative)ses.getAttribute("beanRepresentative" );
		       String id_representative ="0";
		       if(beanRepresentative != null)
		    	   id_representative = String.valueOf( beanRepresentative.getId() );
		    	   
		    	   
			  %>

			
            <html:form action="/doctorChange" enctype="multipart/form-data" >
               <table ALIGN="CENTER" border="0" style="width: 50%;">
				...
				<TR>
					...
					<TD><html:text property="name" size="50" value="<%=beanDoctor.getName()%>"></html:text></TD>
		        	<TD> <FONT COLOR="RED"> <html:errors property="name"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.address" /></TD>
					<TD><html:text property="address" size="50" value="<%=beanDoctor.getAddress()%>"></html:text></TD>					
		        	<TD> <FONT COLOR="RED"> <html:errors property="address"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.email" /></TD>
					<TD><html:text property="email" value="<%=beanDoctor.getEmail()%>"></html:text></TD>					
		        	<TD> <FONT COLOR="RED"> <html:errors property="email"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.patient.notes" /></TD>
					<TD><html:textarea property="notes" cols="50" rows="10" value="<%=beanDoctor.getNotes()%>"/></TD>					
		        	<TD> <FONT COLOR="RED"> <html:errors property="notes"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.phone" /></TD>
					<TD><html:text property="phone" value="<%=beanDoctor.getPhone()%>"></html:text></TD>					
		        	<TD> <FONT COLOR="RED"> <html:errors property="phone"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.phone_cel" /></TD>
					<TD><html:text property="phone_cel" value="<%=beanDoctor.getName()%>"></html:text></TD>					
		        	<TD> <FONT COLOR="RED"> <html:errors property="phone_cel"/></FONT> 
				</TR>
				<TR>
					<TD><bean:message key="label.representative.select" /></TD>
					<TD>	 
					     <html:select property="id_representative">
							<logic:iterate id="id" indexId="indexId" name="listBeanRepresentative" scope="session" >
							<OPTION value= <%="'"%><bean:write name="id" property="id" /><%="'"%>  
							<logic:equal name="id" property="id" value="<%=id_representative%>"> 
									    				SELECTED
							</logic:equal>
		        	...
		        	<TD> <FONT COLOR="RED"> <html:errors property="id_representative"/></FONT> 
				</TR>
