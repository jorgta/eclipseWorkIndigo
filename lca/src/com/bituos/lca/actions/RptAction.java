			...
			try 
		      {
				...
				if ( rptForm.getProcess().equals("quote") )  
					{
				...
				else if ( rptForm.getProcess().equals("quotes") )
				      {
						if (    rptForm.getP1()==null 
							...
							{
							  httpSession.setAttribute("done", "/truckAdmin/rpt/rptQuotes.jsp");
					...
					if ( solo )
					...
					else
					  {
						if (    rptForm.getP1()==null 
						  ...
						  {
						    httpSession.setAttribute("done", "/truckAdmin/rpt/rptSales.jsp");
						    com.bituos.Error e = new com.bituos.Error("Los campos Fecha inicial o Fecha Final no deben ser vacios", request);
					        forward = mapping.findForward("error");
						  }
						else
					...
					if ( solo )
					...
					else
					  {
						if (    rptForm.getP1()==null 
						  ...
						  {
						    httpSession.setAttribute("done", "/truckAdmin/rpt/rptContracts.jsp");
						    com.bituos.Error e = new com.bituos.Error("Los campos Fecha inicial o Fecha Final no deben ser vacios", request);
					        forward = mapping.findForward("error");
						  }
						else
				...
				else if ( rptForm.getProcess().equals("rptOrder") ) 
					  {
						
					    int order_id = Integer.valueOf( rptForm.getP1() );
					    httpSession.setAttribute( "crystal_rptname", new String("rptOrder.rpt") ) ;
						httpSession.setAttribute( "parameterCount", new Integer( 1 ) ) ;
						
						httpSession.setAttribute( "parameter1", new Integer(  rptForm.getP1() ) ) ;
						httpSession.setAttribute( "parameterType1", new String("Integer") ) ;
						httpSession.setAttribute( "parameterName1", new String("test_order_id") ) ;
	
						forward = mapping.findForward("printGlobal");
					  }
				else if ( rptForm.getProcess().equals("rptTicket") ) 
				  {
					
				    int order_id = Integer.valueOf( rptForm.getP1() );
				    httpSession.setAttribute( "crystal_rptname", new String("rptTicket.rpt") ) ;
					httpSession.setAttribute( "parameterCount", new Integer( 1 ) ) ;
				...
				else if ( rptForm.getProcess().equals("rptResult") ) 
				  {
					
				    int order_id = Integer.valueOf( rptForm.getP1() );
				    boolean showError = false;
				    //verifica si esta terminado
				    query = " FROM BeanTestOrder u" +
							" WHERE u.id = " + order_id +
					  		" AND u.is_finished = 1";
					...
					if ( list.size() == 0 )
					    {
							com.bituos.Error e = new com.bituos.Error("La orden no tiene parametros capturados.", request);
							showError = true;
					    }
				    ...
				    }else
				    {
				    	...
				    	httpSession.setAttribute("url", new String("pre.jsp?&process='rptResult'&action='pre.do'&target=''"));
						
				    }
					
					/*httpSession.setAttribute( "parameter2", new Integer(  rptForm.getP1() ) ) ;
					httpSession.setAttribute( "parameterType2", new String("Integer") ) ;
					httpSession.setAttribute( "parameterName2", new String("id_order") ) ;*/

					
				  }
				else if ( rptForm.getProcess().equals("rptOrderResult") ) 
				  {
				    ...
				    httpSession.setAttribute( "crystal_rptname", new String(rptForm.getP1()) ) ;
					httpSession.setAttribute( "parameterCount", new Integer( 1 ) ) ;
					
					httpSession.setAttribute( "parameter1", new Integer( rptForm.getP2()  ) ) ;
					httpSession.setAttribute( "parameterType1", new String("Integer") ) ;
				...
				else if ( rptForm.getProcess().equals("rptOrders") ) 
				  {
					...
					if ( rptForm.getP1().equals("") 
						 ...
						 rptForm.getP2().equals("") 
						  ...
						  {
							httpSession.setAttribute("done", "/lca/rpt/rptOrders.jsp");
							com.bituos.Error e = new com.bituos.Error("Se deben especificar ambas fechas.", request);
						    forward = mapping.findForward("error");
						  }
						else
						  {
							...
							if ( dateBegin.getTime() > dateEnd.getTime())
							  ...
							  {
								int id_brach_capture;
								if(Integer.valueOf( rptForm.getP5() ).equals(2))
								{
									id_brach_capture = 0;
								}
								else
								{
									id_brach_capture = Integer.valueOf( rptForm.getP3() );
								}
								
								
								
								int id_branch_delivery = 0;
								
								if(Integer.valueOf( rptForm.getP7() ).equals(2))
								{
									id_branch_delivery = 0;
								}
								else
								{
									id_branch_delivery = Integer.valueOf( rptForm.getP6() );
								}
								
							    httpSession.setAttribute( "crystal_rptname", new String("rptOrders.rpt") ) ;
								httpSession.setAttribute( "parameterCount", new Integer( 4 ) ) ;
													
								httpSession.setAttribute( "parameter1", new Integer( id_brach_capture ) );
								httpSession.setAttribute( "parameterType1", new String("Integer") ) ;
								httpSession.setAttribute( "parameterName1", new String("id_brach_capture") ) ;
								...
								httpSession.setAttribute( "parameter4", new Integer( id_branch_delivery ) );
								httpSession.setAttribute( "parameterType4", new String("Integer") ) ;
								httpSession.setAttribute( "parameterName4", new String("id_branch_delivery") ) ;
