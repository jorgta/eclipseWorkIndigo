 ...
 * @author
 */
public class CopyOfOrderResultForm extends ActionForm {
	...
	public ActionErrors validate(
		...
		if ( process == null)
		  ...
		  {
			int index = 0;
			for(TestResult beanTestResultElement : this.beanTestResults) 
			{
				
				
				String indexFieldValue = "value["+String.valueOf( index ) +"]";
				String indexFieldNameValueStr = "value_str["+String.valueOf( index ) +"]";
				String indexFieldNameDateReg = "date_reg["+String.valueOf( index ) +"]";
				
				 if(beanTestResultElement.getIs_numeric().equals("1") )
				 {
					if ( beanTestResultElement.getValue() != null )
					  {
						beanTestResultElement.setValue( beanTestResultElement.getValue().trim());
						if ( beanTestResultElement.getValue().length() == 0 )
						  errors.add(indexFieldValue, new org.apache.struts.action.ActionError("error.field.required"));
					  }
					else
				      errors.add(indexFieldValue, new org.apache.struts.action.ActionError("error.field.required"));
				 }else
				 {				 
					 if ( beanTestResultElement.getValue_str() != null )
					  {
						beanTestResultElement.setValue_str( beanTestResultElement.getValue_str().trim());
						if ( beanTestResultElement.getValue_str().length() == 0 )
						  errors.add(indexFieldNameValueStr, new org.apache.struts.action.ActionError("error.field.required"));
					  }
					else
				      errors.add(indexFieldNameValueStr, new org.apache.struts.action.ActionError("error.field.required"));
				 }
				...
				/*if ( beanTestResultElement.getValue_str() != null )
				  {
					beanTestResultElement.setValue_str( beanTestResultElement.getValue_str().trim());
					if ( beanTestResultElement.getValue_str().length() == 0 )
					  errors.add(indexFieldNameDateReg, new org.apache.struts.action.ActionError("error.field.required"));
				  }
				else
			      errors.add(indexFieldNameDateReg, new org.apache.struts.action.ActionError("error.field.required"));*/
			...
			if ( is_numeric.equals("1") )
			  {
			    ...
			    {
			    	errors.add("value", new org.apache.struts.action.ActionError("error.field.invalid.format"));
			    }
			  }
			else
				if ( value != null )
				  {
					if ( value.length() == 0 )
					  errors.add("value", new org.apache.struts.action.ActionError("error.field.required"));
				  }
				else
			      errors.add("value", new org.apache.struts.action.ActionError("error.field.required"));
			*/
