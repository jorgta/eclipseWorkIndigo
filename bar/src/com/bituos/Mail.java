			...
			try
			  {
				...
				//BeanControlPanel control_panelxxxx = (BeanControlPanel) sessionH.load( BeanControlPanel.class, new Integer( 1 ) );
				Authenticator authenticator = null;
				HttpSession httpSession = req.getSession();
				BeanSmsSend beanUser = (BeanSmsSend) httpSession.getAttribute("beanUser");
				BeanCompany beanCompany = beanUser.getId_company();
				...
				//String password = control_panel.getPassword();
				 ...
				 props.put("mail.smtp.host", beanCompany.getSmtp_server() );
				...
				javax.mail.Session session = javax.mail.Session.getInstance(props, authenticator);
				 ...
				 message.setFrom(      new InternetAddress( beanCompany.getSmtp_email() ) );
				...
				if ( beanCompany.getSmtp_is_secure().equals("Y") ) 
				  {
					Transport transport = session.getTransport("smtp");
				
					//transport.connect( control_panel.getServer(), control_panel.getUser_name(), password);					
					//transport.connect( beanCompany.getSmtp_server(), beanCompany.getSmtp_user_name(), beanCompany.getSmtp_password() );
					transport.connect( beanCompany.getSmtp_server(),new Integer( beanCompany.getSmtp_port()).intValue(), beanCompany.getSmtp_user_name(), beanCompany.getSmtp_password() );
