...
import contactcenter.forms.SearchForm;
	...
	public ActionForward execute(
		...
		if ( beanUser == null )
			...
			try {
				...
				if ( searchForm.getConcept().equals("client") )  //client
				  {
					...
					else if ( searchForm.getForward().equals("asignSalesAgent") )
					{
					  ...
					  if(list.size()>0)
						...
						{
							com.tecunsa.Error e = new com.tecunsa.Error("Usuario no existe o fue dado de baja.", request);
				...
				else if ( searchForm.getConcept().equals("case") )  //case
				  {
					if ( searchForm.getForward().equals("new") )
					  {
						//Pone en sesión una lista de clientes que son buscados por algun criterio
						
						if ( searchForm.getAditional1().equals("name") )
							query = " FROM BeanClient u" +
									" WHERE u.name LIKE '%" + searchForm.getFilter().toUpperCase() + "%'" +
									" AND u.active='Y'";
						else if ( searchForm.getAditional1().equals("email") )
							query = " FROM BeanClient u" +
									" WHERE u.email LIKE '%" + searchForm.getFilter2() + "%'" +
									" AND u.active='Y'";
							...
							query = " FROM BeanClient u" +
									" WHERE u.id IN ( SELECT uc.id_client.id" +
									"                 FROM BeanUserClient uc" +
									"                 WHERE uc.id_user.name LIKE '%" + searchForm.getFilter4().toUpperCase() + "%'" +
									"                )" +
									" AND u.active='Y'";
