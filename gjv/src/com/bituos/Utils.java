	...
	public static Date Now()
	  {
		...
		c.set(  new Integer( c.get(GregorianCalendar.YEAR)).intValue(),
				new Integer( c.get(GregorianCalendar.MONTH)).intValue(),
				new Integer( c.get(GregorianCalendar.DATE)).intValue(), 
				new Integer( c.get(GregorianCalendar.HOUR)).intValue(), 
				new Integer( c.get(GregorianCalendar.MINUTE)).intValue(), 
				new Integer( c.get(GregorianCalendar.SECOND)).intValue() 
			 );
...
final static String noventa      = "noventa "	   ;
	...
	public static double getTCfromCT( HttpServletRequest request )
	  {
		// obtener TC
		double tc = 0;
		...
		try
		  {
			URL url = new URL("http://www.ctin.com.mx");
			...
			try
			  {
				...
				while ((line = in.readLine()) != null)
				  {
					if ( line.indexOf("Tipo de cambio") > 0 )
					  {
						index = 1;
						break;
					  }
				...
				if ( index == 1 )
				  {
					if ( (line = in.readLine()) != null )
					  if ( (line = in.readLine()) != null )
						if ( (line = in.readLine()) != null )
						  if ( (line = in.readLine()) != null )
							tc = new Double( line.substring(38, 43) ).doubleValue();
				  }
			  }
			catch( Exception e )
			  {
				try
				  {
					  BeanControlPanel cp = (BeanControlPanel) session.load( BeanControlPanel.class, new Integer(1) );
	
//					  new com.bituos.Mail(
//						  cp.getEmail_mine(),
//						  "Hola\n\n"
//							  + "Hay algun problema con buscar el valor de la TC.\n\n"
//							  + "Exception:"
//							  + e.getMessage(),
//						  "Bituos: error para bucar TC.",
//						  request);
				  }
				catch( Exception e1 )
				  {
