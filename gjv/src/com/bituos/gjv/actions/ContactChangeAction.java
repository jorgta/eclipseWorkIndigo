...
import com.bituos.gjv.forms.ContactChangeForm;
 ...
 * @author
 */
public class ContactChangeAction extends Action {
	...
	public ActionForward execute(
		...
		// return value
		ContactChangeForm contactChangeForm = (ContactChangeForm) form;
		...
		if ( beanUser == null )
			...
			try {
				...
				if(contactChangeForm.getProcess().equals("change"))
				{
					...
					BeanContactRH bean = (BeanContactRH) session.load( BeanContactRH.class, new Integer(contactChangeForm.getIdContact()));
				  //BeanInOut beanInOut =(BeanInOut)session.load( BeanInOut.class, new Integer(contactChangeForm.getId()));;
				  
				  String query =  " FROM BeanInOut u" +
					              " WHERE u.idContactRH.id =" + bean.getId();

				  List list =   session.createQuery( query ).list();
				  BeanInOut beanInOut = (BeanInOut) list.get(0);
				 ...
				 // System.out.println(contactChangeForm.getMarried());
				  ...
				  ses.setAttribute("done", "contactsRH/contactChange.jsp" );
