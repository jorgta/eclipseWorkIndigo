...
import com.bituos.gjv.forms.ProcessAdminForm;
			...
			try
			  {
				...
				if ( list.size() == 0 )
				  {
					com.bituos.Error e = new com.bituos.Error("Usted es el unico usuario disponible.", request);
					//Pone en sesión
			...
			try
			  {
				...
				if ( list.size() > 0 )
				  {
					com.bituos.Error e = new com.bituos.Error("Ese perfil ya existe.", request);
					httpSession.setAttribute("done", "javascript:history.back();");
					//forward declarado en struts-config.xml
					forward = mapping.findForward("error");
				  }
			...
			try
			  {
				...
				if ( list.size() == 0 )
				  {
					com.bituos.Error e = new com.bituos.Error("Ese perfil ya fue borrado.", request);
					httpSession.setAttribute("done", "javascript:history.back();");
					//forward declarado en struts-config.xml
					forward = mapping.findForward("error");
				  }
			...
			try
			  {
				...
				System.out.println("idprocess" + processAdminForm.getProfileAsignProcess_id_process());
				System.out.println("action" + processAdminForm.getAction());
				System.out.println("idprofile" + processAdminForm.getProfileAsignProcess_id_profile());
