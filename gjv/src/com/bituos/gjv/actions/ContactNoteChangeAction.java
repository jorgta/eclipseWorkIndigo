...
import com.bituos.gjv.forms.ContactNoteChangeForm;
 ...
 * @author
 */
public class ContactNoteChangeAction extends Action {
	...
	public ActionForward execute(
		...
		// return value
		ContactNoteChangeForm contactNoteChangeForm = (ContactNoteChangeForm) form;
		...
		if ( beanUser == null )
			...
			try {
				...
				if(contactNoteChangeForm.getProcess().equals("change"))
				{
				  ...
				  BeanNotes bean = (BeanNotes) session.load( BeanNotes.class, new Integer(contactNoteChangeForm.getIdNote()));
				  ...
				  ses.setAttribute("done", "contactsRH/contactChangeNote.jsp" );
				...
				if(contactNoteChangeForm.getProcess().equals("select"))
				{
				    ...
				    BeanContactRH bean = (BeanContactRH) session.load( BeanContactRH.class, new Integer(contactNoteChangeForm.getIdContact()));
				...
				if(contactNoteChangeForm.getProcess().equals("selectNote"))
				{
					...
					if ( !list.isEmpty() )
					...
					else
			         {
			    	   ...
			    	   BeanNotes beanNotes= new BeanNotes();
			    	   list.add(beanNotes);	
