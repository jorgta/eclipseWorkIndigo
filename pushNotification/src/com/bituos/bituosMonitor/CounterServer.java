...
public class CounterServer {
	...
	*/
    ...
    /*test DAvid , para ver si llega un arreglo de counter a Rad studio */
	
	public Counter[] getCountersTest( )
	  {
		...
		try
		  {
			...
			String query = " FROM BeanCounter u" +
				           " WHERE u.id_device.id = 2"; 
	
			List list = session.createQuery( query ).list();
			
			if(!list.isEmpty())
				{
				  counterList = new Counter[ list.size() ];
	...
	private BeanCounter[] getCounters(String company, String location, String password, Device device ) throws Exception
	  {
		Config config = new Config(  );
		...
		Session    session = null;
		Transaction tx = null;
		BeanCounter[] counterBeanCounterList = null;
			...
			if ( id_company > 0 )
			  {
				...
				String query = " FROM BeanCounter u" +
						       " WHERE u.id_company.id=" + id_company +
						       " AND u.id_device.id = " + beanDevice.getId(); 
			
				List list = session.createQuery( query ).list();
				
				if(!list.isEmpty())
				{
				  counterBeanCounterList = new BeanCounter[list.size()];
				  ...
				  while ( iter.hasNext() )
				    {					
					  BeanCounter beanCounter = (BeanCounter) iter.next();	
					  counterBeanCounterList[index] =beanCounter; 
					  index++;				
				    }
				 }
			  }
	...
	public ServiceConfiguration getConfiguration(String companyEncrypted, String locationEncrypted, String passwordEncrypted, Device device) throws Exception
	  {
		...
		try
		  {
			...
			String query =  " FROM BeanCouterDetail u" +
							" WHERE u.name='" + propertyName + "'" + 
							" AND u.name.name_type_counter_detail='" + className + "'"; 
